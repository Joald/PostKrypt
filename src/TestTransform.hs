...
testCompTrans v1 v2 p =
  trpoint (translate (vec v1) >< translate (vec v2)) (point p) === trpoint (translate (vec v1 >< vec v2)) (point p)

testCompRot :: R -> R -> R2 -> Property
testCompRot r1 r2 p = trpoint (rotate r1 >< rotate r2) (point p) === trpoint (rotate (r1 + r2)) (point p)

testAssRot :: R -> R -> R -> R2 -> Property
testAssRot a b c p =
  trpoint (rotate a >< (rotate b >< rotate c)) (point p) === trpoint ((rotate a >< rotate b) >< rotate c) (point p)
...
instance Arbitrary Transform where
  arbitrary = sized arb
    where
      arb 0 = return m1
      arb 1 = oneof [transGen, rotate <$> arbitrary]
      arb n = liftM2 (><) arb2 arb2
        where
          arb2 = arb (n `div` 2)
